 ...
 */
interface LoansStackPlainProps extends cdk.StackProps {
  vpc: ec2.IVpc;
  eventBus: events.EventBus;
}

/**
 * Example implementation of the LoansStack without using a custom Layer 3 Construct.
 * 
 * @deprecated This stack is intended for demonstration purposes only and should not be used.
 */
export class LoansStackPlain extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: LoansStackPlainProps) {
  ...
  private createLoanHandlers(props: LoansStackPlainProps, handlerPath: string) {
    const handlers = {
  ...
  private createLambda(id: string, handler: string, props: LoansStackPlainProps, handlerPath: string): lambda.Function {
